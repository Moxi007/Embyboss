...
class Embyservice(metaclass=Singleton):
    ...
    async def emby_cust_commit(self, emby_id: str = None, days: int = 7, method: str = None) -> Optional[List[Dict]]:
        ...
        try:
            ...
            if method == 'sp':
                final_sql = f"SELECT UserId, SUM(COALESCE(PlayDuration, 0) - COALESCE(PauseDuration, 0)) AS WatchTime FROM PlaybackActivity WHERE DateCreated >= '{start_time}' AND DateCreated < '{end_time}' GROUP BY UserId ORDER BY WatchTime DESC"
            else:
                final_sql = f"SELECT MAX(DateCreated) AS LastLogin, SUM(COALESCE(PlayDuration, 0) - COALESCE(PauseDuration, 0)) / 60 AS WatchTime FROM PlaybackActivity WHERE UserId = '{emby_id}' AND DateCreated >= '{start_time}' AND DateCreated < '{end_time}' GROUP BY UserId"
    ...
    async def get_emby_report(self, types: str = 'Movie', emby_id: str = None, days: int = 7, 
        ...
        try:
            ...
            sql_parts = [
                ...
                "SUM(COALESCE(PlayDuration, 0) - COALESCE(PauseDuration, 0)) AS total_duarion",
