...
from sqlalchemy import Column, Integer, String, DateTime, UniqueConstraint, select, delete
    ...
    try:
        async with Session() as session:
            if is_favorite:
                ...
                records_to_delete = list(result.scalars().all())
    ...
    try:
        async with Session() as session:
            await session.execute(
                delete(EmbyFavorites).filter(EmbyFavorites.embyname == emby_name)
            )
            await session.commit()
        return True
    except Exception as e:
