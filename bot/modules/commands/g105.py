...
active_105_games: Dict[int, Dict] = {}
...
class Game105Logic:
    ...
    def create_deck() -> List[str]:
        ...
        """
        # 创建完整牌组
        deck = [f"{suit}{rank}" for suit in Game105Logic.SUITS for rank in Game105Logic.RANKS]
    ...
    def dealer_auto_draw(dealer_cards: List[str], deck: List[str]) -> List[str]:
        ...
        """
        while True:
            points = Game105Logic.calculate_points(dealer_cards)
            ...
            # 抽牌
            if deck:
                card = Game105Logic.deal_card(deck)
                if card:
...
def generate_game_message(game: dict, show_dealer_cards: bool = False) -> str:
    ...
    player_points = Game105Logic.calculate_points(player_cards)
    dealer_points = Game105Logic.calculate_points(dealer_cards)
...
def create_game_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("🎴 要牌", callback_data=f"105_hit_{user_id}"),
            InlineKeyboardButton("🛑 停牌", callback_data=f"105_stand_{user_id}")
        ]
    ])
...
def parse_105_command(command_text: str) -> dict:
    """
    解析 /105 命令
...
async def cleanup_game_state(user_id: int, refund: bool = False):
    """
    清理游戏状态，可选退款
    ...
    """
    try:
        if user_id in active_105_games:
            game = active_105_games[user_id]
...
async def handle_timeout(user_id: int, game: dict):
    """
    处理超时
    ...
    try:
        ...
        # 清理游戏状态（不退款）
        await cleanup_game_state(user_id, refund=False)
        
    except Exception as e:
        LOGGER.error(f"处理超时失败: {e}")


async def start_timeout_timer(user_id: int, timeout_seconds: int = 60):
    """
    启动超时计时器
    ...
    if user_id in active_105_games:
        game = active_105_games[user_id]
        await handle_timeout(user_id, game)


async def settle_game(user_id: int, game: dict, result: dict):
    """
    结算游戏
    ...
    try:
        ...
        # 发生错误时尝试退款
        await cleanup_game_state(user_id, refund=True)
...
async def handle_105_command(client, message):
    ...
    try:
        # 1. 解析命令
        parse_result = parse_105_command(message_text)
        ...
        # 7. 创建牌堆并发牌
        deck = Game105Logic.create_deck()
        player_cards = [Game105Logic.deal_card(deck), Game105Logic.deal_card(deck)]
        dealer_cards = [Game105Logic.deal_card(deck), Game105Logic.deal_card(deck)]
        ...
        # 存储游戏状态
        active_105_games[user_id] = game_state
        
        # 10. 启动超时计时器
        asyncio.create_task(start_timeout_timer(user_id, 60))
...
@bot.on_callback_query(filters.regex(r"^105_(hit|stand)_"))
async def handle_105_callback(client, call):
    """处理游戏按钮回调"""
    try:
        # 解析回调数据
        ...
        # 检查游戏是否存在
        if target_user_id not in active_105_games:
            await call.answer("❌ 游戏已结束", show_alert=True)
            return
        
        game = active_105_games[target_user_id]
        ...
        if action == 'hit':
            # 要牌
            card = Game105Logic.deal_card(game['deck'])
            if card:
                game['player_cards'].append(card)
            
            player_points = Game105Logic.calculate_points(game['player_cards'])
            ...
            if player_points > 10.5:
                # 爆牌，游戏结束
                result = Game105Logic.judge_winner(
                    player_points,
                    Game105Logic.calculate_points(game['dealer_cards']),
                    game['player_cards'],
                    game['dealer_cards']
                )
                await settle_game(target_user_id, game, result)
            ...
            if len(game['player_cards']) == 5 and player_points <= 10.5:
                # 五小龙，触发庄家抽牌
                game['dealer_cards'] = Game105Logic.dealer_auto_draw(
                    game['dealer_cards'],
                    game['deck']
                )
                dealer_points = Game105Logic.calculate_points(game['dealer_cards'])
